...
class CheckExistingRecordTypes(Deploy):
    """
    ...
    def _run_task(self):
        if self.can_deploy_opp_rts():
            super()._run_task()

    def can_deploy_opp_rts(self):
        business_process_exists = self.business_process_exists()
        record_types_exist = self.record_types_exist()
        can_deploy = business_process_exists and not record_types_exist
        self.logger.info(f"{'Deploying...' if can_deploy else 'Skipping...'}")
        return can_deploy
